pipeline {
    ...
    post {
        failure {
            discordSend description: "Server FAILED to Deploy/Destroy", footer: "Server-Deploy-CI/CD Failed", notes: "Log: ${currentBuild.rawBuild.getLog(10)}", link: env.BUILD_URL, result: currentBuild.currentResult, title: JOB_NAME, webhookURL: env.SOCAAS_WEBHOOK
        }
    }  
}
